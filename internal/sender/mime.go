...
func BuildMIME(email *Email, defaultFrom, defaultFromName string) ([]byte, error) {
	...
	if len(regularAttachments) > 0 {
		...
		if len(inlineAttachments) > 0 {
			...
			subRelWriter := multipart.NewWriter(part)
			_ = subRelWriter.SetBoundary(relWriter.Boundary())
			if err := writeRelatedBody(subRelWriter, email, inlineAttachments); err != nil {
				return nil, err
			}
		} else {
...
func writeRelatedBody(relWriter *multipart.Writer, email *Email, inline []*Attachment) error {
